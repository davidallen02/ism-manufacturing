...
quad <- function(a, b, c, d, plot.title, start.date = '2000-01-01'){
  p <- dplyr::left_join(read_data(a), 
                        read_data(b), 
                        by = 'date') %>% 
    dplyr::left_join(read_data(c), by = 'date') %>% 
    dplyr::left_join(read_data(d), by = 'date') %>% 
    ...
    ggplot2::theme(
      plot.title = ggplot2::element_text(size = ggplot2::rel(2), face = 'bold'), 
      axis.title = ggplot2::element_blank(), 
      axis.text = ggplot2::element_text(size = ggplot2::rel(1.25)), 
      plot.caption = ggplot2::element_text(size = ggplot2::rel(1)),  
      strip.text = ggplot2::element_text(size = ggplot2::rel(1.25), color = 'white'), 
